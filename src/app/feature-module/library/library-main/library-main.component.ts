...
export class LibraryMainComponent implements OnInit {
  ...
  private getTableData(): void {
    ...
    this.data.library.map((res: library, index: number) => {
      const serialNumber = index + 1;
      if (index >= this.skip && serialNumber <= this.limit) {
        res.id = serialNumber;
        this.lstLibrary.push(res);
        this.serialNumberArray.push(serialNumber);
      }
    });
    this.dataSource = new MatTableDataSource<library>(this.lstLibrary);
    this.totalData = this.data.library.length;
    this.calculateTotalPages(this.totalData, this.pageSize);
  }

  public sortData(sort: Sort) {
    const data = this.lstLibrary.slice();

    if (!sort.active || sort.direction === '') {
      this.lstLibrary = data;
    } else {
      this.lstLibrary = data.sort((a, b) => {
  ...
  public searchData(value: string): void {
    ...
    this.lstLibrary = this.dataSource.filteredData;
  }
